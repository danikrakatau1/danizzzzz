...
const gear200=read('js/gear-action-capsule-v200.js');
...
const checks=[
 ...
 ['V200 capsule boot idempotent',gear200.includes("if(old.dataset.v200==='1')return")],
 ['V200 capsule click owner singular',count(gear200,/gear\.addEventListener\('click'/g)===1],
 ['V200 outside pointer owner singular',count(gear200,/document\.addEventListener\('pointerdown'/g)===1],
 ['V200 keyboard owner singular',count(gear200,/document\.addEventListener\('keydown'/g)===1],
 ['V200 DOM ready once-only',gear200.includes("addEventListener('DOMContentLoaded',boot,{once:true})")],
 ['Product Center no touch navigation listeners',!/addEventListener\(['\"]touch(?:start|move|end|cancel)['\"]/.test(product71)],
 ['Product Center no wheel interception',!/addEventListener\(['\"]wheel['\"]/.test(product71)],
 ['Product Center no history event ownership',!product71.includes('history.pushState')&&!product71.includes('history.replaceState')],
...
console.log('V266 EVENT LISTENER AUDIT PASS — V259 legacy iOS listener runtime retired; V200 capsule listener boot idempotent; Product Center avoids global touch/wheel/history ownership; no duplicate drawer click owners; sidebar wheel interception remains uninitialized; lifecycle bindings are once/idempotent; touch/wheel passive policy constrained; Research/Fee/Profit submit paths single-owned');
